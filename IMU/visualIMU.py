...
label(pos=vec(0,-2.5,0), color=color.blue, text='<b>IVE(LWL) - Department of Engineering<b>', height=50)
...
breadBoard = box(length=2.5, width=5, height=.2, opacity=.8, pos=vector(0,0,0,))
...
while (True):
    try:
        ...
        pitch = float(splitPacket[7])*2*np.pi/360.
